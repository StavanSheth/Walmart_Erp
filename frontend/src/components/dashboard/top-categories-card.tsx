  ...
  return (
    <div className="glass-card rounded-2xl p-5 flex flex-col justify-between h-full">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm sm:text-base font-bold text-slate-900 tracking-tight flex items-baseline gap-1.5">
          <span>Top Categories</span>
          <span className="text-xs font-normal text-slate-500">(By Stock Value)</span>
        </h3>
        <Link
          href="/inventory"
      ...
      {isLoading ? (
        <div className="space-y-4 py-2">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="animate-pulse flex items-center gap-3">
              <div className="w-6 h-6 rounded bg-slate-200/70" />
              <div className="w-24 h-3.5 bg-slate-200/70 rounded" />
              <div className="flex-1 h-2 bg-slate-200/70 rounded-full" />
              <div className="w-16 h-3.5 bg-slate-200/70 rounded" />
            </div>
          ))}
        </div>
      ) : topList.length === 0 ? (
        ...
        <div className="space-y-3.5 my-auto">
          {topList.map((cat, idx) => {
            ...
            return (
              <div key={cat.categoryId || idx} className="flex items-center gap-3">
                {/* Category Icon */}
                ...
                {/* Category Name */}
                <div className="w-24 sm:w-28 text-xs sm:text-sm font-semibold text-slate-800 truncate" title={cat.categoryName}>
                  {cat.categoryName}
                </div>

                {/* Relative progress bar */}
                <div className="flex-1 bg-slate-100/80 rounded-full h-2 overflow-hidden">
                  <div
                ...
                {/* Stock Value */}
                <div className="w-24 text-right text-xs sm:text-sm font-bold text-slate-800 tabular-nums">
